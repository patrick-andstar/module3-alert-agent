...
const DefaultDecisionSystemPrompt = `你是终端敏感数据外发监控系统的误报分析 Agent。
...
event_id, is_false_positive, agent_verdict, new_risk_level, false_positive_reason, confidence, recall_score, explanation。
...
func LoadSystemPrompt(path string) (string, error) {
	if path == "" {
		return DefaultDecisionSystemPrompt, nil
	}
